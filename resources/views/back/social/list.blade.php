...
@section('content')
        ...
        <div class="row">
            <div class="card card-primary col-md-12 mx-auto" style="padding: 0">
                <div class="card-header">
                    <a href="{{ url('admin/social/add') }}" class="btn btn-block btn-light text-dark"
                        href="{{ url('admin/social/add') }}" title="Thêm"
                        ...
                        <thead>
                            <tr>
                                ...
                                <th class="sorting text-center" tabindex="0" aria-controls="example2" rowspan="1"
                                    colspan="1" aria-label="Browser: activate to sort column ascending">Tên mạng xã hội
                                </th>
                                <th class="sorting text-center" tabindex="0" aria-controls="example2" rowspan="1"
                                    colspan="1" aria-label="Platform(s): activate to sort column ascending">Trạng thái
                                </th>
                                <th class="sorting text-center" tabindex="0" aria-controls="example2" rowspan="1"
                                    colspan="1" aria-label="Engine version: activate to sort column ascending">Sắp xếp
                                </th>
                                <th class="sorting text-center" tabindex="0" aria-controls="example2" rowspan="1"
                        ...
                        <tbody>
                            ...
                            @if (isset($social) && count($social) > 0)
                                @foreach ($social as $socialData)
                                    <tr class="odd">
                                        <td>{{ $count++ }}</td>
                                        <td>{{ $socialData->Name }}</td>
                                        <td>{{ $socialData->Status == 1 ? 'Bật' : 'Tắt' }}</td>
                                        <td>{{ $socialData->Sort }}</td>
                                        <td class="text-center">
                                            <a href="{{ url('admin/social/edit/' . $socialData->RowID) }}" title="Chỉnh sửa">
                                                <i class="fa-solid fa-pen-to-square" style="color: #db0b0b;"></i>
                                            </a>
                                            <a href="{{ url('admin/social/delete/' . $socialData->RowID) }}" title="Xoá"
    ...
    <script>
        function confirmDelete() {
            var result = confirm("Bạn có chắc chắn muốn xóa mạng xã hội này?");
            return result;
        }
    </script>
